import {
    ...
    initialState as promoInitialState
...
const mockPromo3: Promotion = {
    id: "p3",
    title: "Promo Elec Especial",
    description: "Desc",
    store: mockStore1,
    category: mockCategory1,
    imageUrl: "",
...
const mockPromo4: Promotion = {
    id: "p4",
    title: "Promo Lejos",
    description: "Desc",
    store: { id: "store-3", name: "Tienda C", latitude: 20, longitude: 20 },
    category: mockCategory1,
    imageUrl: "",
    isSpecial: false
};
...
const mockedFetchPromotions = fetchPromotions as jest.MockedFunction<
    typeof fetchPromotions
>;
const mockedFetchCategories = fetchCategories as jest.MockedFunction<
    typeof fetchCategories
>;
const mockedFetchStores = fetchStores as jest.MockedFunction<
    typeof fetchStores
>;
...
describe("promoStore", () => {
    beforeEach(() => {
        usePromoStore.setState(
            { ...usePromoStore.getState(), ...promoInitialState },
            true
        );
        resetApiMocks();
        jest.clearAllMocks();
    ...
    describe("selectFilteredPromotions", () => {
        it("should filter promotions by filters", () => {
            ...
            expect(
                filtered.every(
                    (p) => p.category.id === "cat-1" && p.store.id === "store-1"
                )
            ).toBe(true);
        });
    });
